...
\begin{table}[h]
\centering
\caption{Full-Stage Results (Gemini Flash, n=115)}
\label{tab:full}
\begin{tabular}{@{}lr@{}}
\toprule
\textbf{Metric} & \textbf{Score} \\
...
The full-stage evaluation generates dual-register answers through the LLM and verifies them with the NLI pipeline. Due to API rate limits during evaluation, we also ran the pipeline on a local LLM (Qwen2.5-3B via Ollama) to validate end-to-end functionality.

%% ======================================================================
\section{Discussion}
\label{sec:discussion}

\textbf{Temporal routing as a legal requirement.} Our temporal resolver achieves 0.991 accuracy---the single error being a tax-domain edge case where the IT Act 2025's effective date is ambiguous for certain provisions. This near-perfect accuracy is because temporal routing is implemented as a deterministic mapping (date $\rightarrow$ regime) rather than an LLM-dependent classification, making it both reliable and auditable.
